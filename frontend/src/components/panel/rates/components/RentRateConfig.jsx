...
const RentRateConfig = ({
    ...
    // Obtener tarifas de arriendo del estado
    const rentRates = rates.all?.filter(rate => rate.type === RATE_TYPES.RENTAL_RATE) || [];
    const activeRentRates = rentRates.filter(rate => rate.active);
    const currentRateData = activeRentRates.find(rate => rate.isCurrentlyActive) || activeRentRates[0];

    useEffect(() => {
        // Resetear formulario cuando cambie el modo de edición
        if (!showForm) {
            setFormData({
                type: RATE_TYPES.RENTAL_RATE,
                dailyAmount: '',
    ...
    const validateForm = () => {
        ...
        // Validar monto
        const amountError = validateAmount(parseFloat(formData.dailyAmount), RATE_TYPES.RENTAL_RATE);
        if (amountError) {
        ...
        // Validar fecha de inicio no sea en el pasado (solo para nuevas tarifas)
